...
double LaserScanner::getScan()
{
  ...
  double laser_x{ max_laser_depth * cos(robot_heading) };
  double laser_y{ max_laser_depth * sin(robot_heading) };
  ...
  //line_seg_intersector_ =
      //new osgUtil::LineSegmentIntersector(laser_start, laser_end);
  //osgUtil::IntersectionVisitor iv(line_seg_intersector_.get());
  //osg_scene_->accept(iv);

  std::cout << "Contains intersections? "
            << line_seg_intersector_->containsIntersections() << std::endl;
  osg::Vec3d first_int =
  ...
  //first_int =
      //line_seg_intersector_->getFirstIntersection().getLocalIntersectPoint();
  //std::cout << "first int: " << first_int[0] << ", " << first_int[1] << ", "
    //<< first_int[2] << std::endl;
